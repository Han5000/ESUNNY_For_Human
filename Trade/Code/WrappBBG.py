...
class BLP:
    ...
    def __init__(self, sapi_dic=None):
        if sapi_dic is not None:
            self.session = BLPSession(sapi_dic['host_ip'], sapi_dic['host_port'], sapi_dic['uuid'],
                                      sapi_dic['local_ip'])
        else:
...
class BLPTS:
    ...
    def __init__(self, securities=[], fields=[], **kwargs):
        ...
        if 'sapi_dic' in self.kwargs and self.kwargs['sapi_dic'] is not None:
            self.session = BLPSession(kwargs['sapi_dic']['host_ip'], kwargs['sapi_dic']['host_port'],
                                      kwargs['sapi_dic']['uuid'], kwargs['sapi_dic']['local_ip'])
        else:
            self.session = BLPSession()
        self.session.openService('//BLP/refdata')
        self.refDataSvc = self.session.getService('//BLP/refdata')
...
class BLPStream(threading.Thread):
    ...
    def __init__(self, strSecurityList=['ESZ9 Index', 'VGZ9 Index'], strDataList=['BID', 'ASK'], floatInterval=0,
                 intCorrIDList=[0, 1], sapi_dic=None):
        threading.Thread.__init__(self)
        if sapi_dic is not None:
            self.session = BLPSession(
                BLPSession(sapi_dic['host_ip'], sapi_dic['host_port'], sapi_dic['uuid'], sapi_dic['local_ip']))
        else:
            self.session = BLPSession()
...
def simpleHistoryRequest(securities=[], fields=[], startDate=datetime.datetime(2015, 1, 1),
                         endDate=datetime.datetime(2016, 1, 1), **kwargs):
    '''
